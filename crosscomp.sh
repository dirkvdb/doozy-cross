...
if [ "$#" -ne 1 ]; then
    echo "No toolchain provided. Choices: archarmv6|archarmv7|macv6|android"
    exit 1
fi
...
mkdir build
cd build
checkresult cmake -DCMAKE_TOOLCHAIN_FILE=../cross/toolchain-${ARMARCH}.make -DCMAKE_BUILD_TYPE=Release -DSTATIC_BINARY=ON ../doozy
checkresult make -j4
